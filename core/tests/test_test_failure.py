...
from aios.services.test_failure import FailureConfidence, FailureSeverity
...
def test_failure_analyzer_classification():
    ...
    assert (
        analyzer.classify_failure("AssertionError: custom msg").pattern_name == "assertion_failure"
    )
    assert (
        analyzer.classify_failure("ModuleNotFoundError: No module").pattern_name == "import_failure"
    )
...
def test_failure_analysis_service_flow(dummy_failed_summary):
    ...
    report = service.diagnose_failures(
        workspace_id="ws_1", execution_summary=dummy_failed_summary, code_summary=summary
    )
