...
def test_strategy_keyword_routing():
    ...
    assert service._select_strategy("Refactor the Event Bus module") == ReasoningStrategy.ENGINEERING
    
    # 2. Research strategy matching
    assert service._select_strategy("Find active academic papers on LLM agents") == ReasoningStrategy.RESEARCH
    
    # 3. Career strategy matching
    assert service._select_strategy("Compare my resume against senior developer jobs") == ReasoningStrategy.CAREER
    
    # 4. Automation strategy matching
    assert service._select_strategy("Deploy a custom webhook workflow on n8n") == ReasoningStrategy.AUTOMATION
