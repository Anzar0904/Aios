...
def test_notion_provider_http_calls(mock_urlopen):
    ...
    mock_resp.read.return_value = json.dumps({"object": "user", "name": "AI OS Bot"}).encode(
        "utf-8"
    )
    ...
    mock_create_resp.read.return_value = json.dumps(
        {
            ...
            "properties": {"title": {"title": [{"text": {"content": "Sample Doc"}}]}},
        }
    ).encode("utf-8")
...
def test_subsystem_integrations_mocked(mock_personal_service):
    ...
    daily_review = LocalDailyReview(
        model_mock, personal_mock, None, registry, MagicMock(), MagicMock()
    )
    summary = DailyReviewSummary(
        completed_tasks=["Task A"], incomplete_tasks=["Task B"], productivity_summary="Great day."
    )
