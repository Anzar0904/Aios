...
def test_omniroute_semantic_context(kernel_setup):
    registry = kernel_setup.registry
    sem_mgr = registry.get(SemanticMemoryManager)
    
    # Clear documentation collection to guarantee seeded point is within retrieval limit
    sem_mgr._get_repository("documentation_memory").clear()
    
    # Seed a memory
    sem_mgr.index_memory(
        "documentation_memory",
        "omni_seed",
        "Critical warning: Gemini API key expires tomorrow.",
        {"workspace_id": "default"},
        ["gemini"]
    )
    
    from aios.services.model_impl import OmniRouteProvider
    from aios.services.model import LLMRequest
    
    provider = OmniRouteProvider(offline_mode=False)
    
    req = LLMRequest(
        prompt="Check Gemini API status",
        system_instruction="Analyze routing",
        model_name="claude-3-5-sonnet"
    )
    
    # Mock the actual HTTP call to prevent network requests
    import unittest.mock as mock
    ...
    mock_response.json.return_value = {
        "choices": [{
            "message": {
                "role": "assistant",
                "content": "Routing analysis completed."
            }
        }]
    }
    ...
    with mock.patch("httpx.Client.post", return_value=mock_response) as mock_post:
        ...
        assert "Enriched Context:" in req.prompt
        assert "Gemini API key expires tomorrow" in req.prompt
