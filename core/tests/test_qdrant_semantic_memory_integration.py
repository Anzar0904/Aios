...
def kernel_setup(monkeypatch):
    ...
    monkeypatch.setattr(
        DocumentationMetadataRepositoryImpl, "save", mock.Mock(return_value=mock_success_res)
    )
    monkeypatch.setattr(
        EngineeringTaskRepositoryImpl, "save", mock.Mock(return_value=mock_success_res)
    )
    ...
    monkeypatch.setattr(
        ...
        "sync_document",
        mock.Mock(
            return_value=KnowledgeSyncResult(
                document_id="mock_doc_id", provider="notion", status="success"
            )
        ),
    )
    ...
    def mock_index_memory(
        ...
        if repo:
            ...
            try:
                return original_index_memory(
                    self, repository_name, entity_id, text, metadata, tags, importance_override
                )
            finally:
...
def test_conversation_indexing(kernel_setup):
    ...
    conv = conv_manager.create_conversation(
        title="Test Semantic Dialog Unique", agent_name="test_agent"
    )
    conv_manager.add_message(
        conv.id, "user", f"How do I query the database dialog semantic {unique_id}?"
    )
...
def test_developer_agent_semantic_memory(kernel_setup):
    ...
    if model_svc:
        model_svc.execute_request = mock.Mock(
...
def test_context_service_enriched_context(kernel_setup):
    ...
    sem_mgr.index_memory(
        "workspace_memory",
        "ctx_seed_ws",
        "Workspace active files: index.css, main.js",
        {"workspace_id": "test_ctx"},
        ["files"],
    )
    sem_mgr.index_memory(
        "conversation_memory",
        "ctx_seed_conv",
        "User preference: Prefer HSL colors",
        {"workspace_id": "test_ctx"},
        ["preference"],
    )
    sem_mgr.index_memory(
        "engineering_memory",
        "ctx_seed_eng",
        "Completed task: Configured Tailwind",
        {"workspace_id": "test_ctx"},
        ["tailwind"],
    )
