...
def test_migration_guide_generation():
    ...
    guide = generator.generate_migration_guide(
        version_from="0.9.0",
        version_to="1.0.0",
        instructions=["Upgrade Python runtime environment to 3.10+", "Run notion-db migration script"]
    )
...
def test_upgrade_guide_generation():
    ...
    guide = generator.generate_upgrade_guide(
        ...
        checklist=["Git pull origin main", "Poetry install", "Agy restart"]
    )
...
def test_validation(mock_memory_service):
    ...
    # 1. Valid artifact
    artifact = ReleaseArtifact("release_notes_1.0.0", "ws_1", "1.0.0", "stable", "# Features\n# Bug Fixes\n# Breaking Changes", 0.0)
    errors = validator.validate_release_document(artifact)
    assert len(errors) == 0
    
    # 2. Invalid semantic version
    artifact_bad_version = ReleaseArtifact("notes_v1", "ws_1", "v1.0", "stable", "# Features\n# Bug Fixes\n# Breaking Changes", 0.0)
    ...
    artifact_dup = ReleaseArtifact("release_notes_1.0.0", "ws_1", "1.0.0", "stable", "# Features\n# Bug Fixes\n# Breaking Changes", 0.0)
...
def test_profile_integration(mock_memory_service, mock_profile_service):
    ...
    assert prefs["section_ordering"] == ["Feature Summary", "Bug Fix Summary", "Breaking Changes", "Validation Summary"]
...
def test_memory_integration(mock_memory_service, mock_profile_service):
    ...
    assert "some code" not in content  # Ensuring repository source code / raw doc is not in the text
