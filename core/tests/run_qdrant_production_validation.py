    ...
    batch_pts = [
        {
            "id": batch_ids[0],
            "vector": [0.2] * 384,
            "payload": {"text": "Batch item 1", "workspace_id": "ws-crud-1"},
        },
        {
            "id": batch_ids[1],
            "vector": [0.3] * 384,
            "payload": {"text": "Batch item 2", "workspace_id": "ws-crud-1"},
        },
    ]
...
def run_search_tests(
    sem_mgr: SemanticMemoryManager, hybrid_svc: HybridRetrievalService
) -> Dict[str, Any]:
    ...
    sem_mgr.index_memory(
        ...
        {"workspace_id": ws1_id, "project_id": "p1"},
        ["security", "db"],
    )
    sem_mgr.index_memory(
        col,
        ...
        {"workspace_id": ws1_id, "project_id": "p2"},
        ["network", "routing"],
    )
    sem_mgr.index_memory(
        col,
        "s_seed_3",
        "Explain local context caching",
        {"workspace_id": ws2_id, "project_id": "p1"},
        ["context", "cache"],
    )
    ...
    has_ws1_only = (
        any("security" in t for t in ws1_texts)
        and any("routing" in t for t in ws1_texts)
        and not any("context" in t for t in ws1_texts)
    )
...
def run_performance_benchmarks(
    sem_mgr: SemanticMemoryManager, engine: EmbeddingEngine, hybrid_svc: HybridRetrievalService
) -> Dict[str, Any]:
...
def compile_reports(
    ...
    latency_str = (
        f"{conn_res['readyz']['latency_ms']:.2f}ms"
        if "latency_ms" in conn_res["readyz"]
        else "FAILED"
    )
