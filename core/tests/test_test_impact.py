...
def mock_code_summary():
    return CodeStructureSummary(
        ...
        call_graph={},
        dependency_graph={
            "core/src/aios/kernel.py": ["core/src/aios/bootstrap.py", "core/src/aios/services/memory.py"],
...
def test_risk_assessment(mock_code_summary):
    ...
    # 1. Non-critical risk
    res_std = analyzer.analyze_change_impact("ws_1", "Update documentation", ["core/src/aios/services/memory.py"], mock_code_summary)
    assert res_std.risk_assessment.overall_risk == "Medium"
    
    # 2. Critical file changes risk (kernel.py)
    res_crit = analyzer.analyze_change_impact("ws_1", "Alter boot sequence", ["core/src/aios/kernel.py"], mock_code_summary)
...
def test_coverage_targets(mock_code_summary):
    ...
    result = analyzer.analyze_change_impact("ws_1", "Optimize code", ["core/src/aios/services/memory.py"], mock_code_summary)
...
def test_backward_compatibility():
    class CustomAnalyzer(LocalChangeImpactAnalyzer):
        def analyze_change_impact(self, workspace_id, objective, affected_files, code_summary):
            res = super().analyze_change_impact(workspace_id, objective, affected_files, code_summary)
