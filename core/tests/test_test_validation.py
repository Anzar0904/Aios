...
def test_validation_gate_evaluators(mock_exec_summary, mock_coverage_report, mock_failure_report):
    ...
    report = service.synthesize_validation(
        "ws_1", mock_exec_summary, mock_coverage_report, mock_failure_report
    )
    ...
    report_warn = service.synthesize_validation(
        "ws_1", mock_exec_summary, mock_coverage_report, mock_failure_report
    )
...
def test_weighted_scoring_model(mock_exec_summary, mock_coverage_report, mock_failure_report):
    ...
    report_low = service.synthesize_validation(
        "ws_1", mock_exec_summary, mock_coverage_report, mock_failure_report
    )
...
def test_backward_compatibility():
    class CustomService(LocalValidationService):
        def synthesize_validation(
            self, workspace_id, execution_summary, coverage_report, failure_report
        ):
            report = super().synthesize_validation(
                workspace_id, execution_summary, coverage_report, failure_report
            )
