...
class LocalApprovalHistoryValidator(ApprovalHistoryValidator):
    ...
    def validate_report(self, report: ApprovalHistoryReport) -> List[str]:
        ...
        if not (0.0 <= stats.average_confidence <= 1.0):
            errors.append(f"Statistics Integrity Error: Average confidence '{stats.average_confidence}' stands outside [0.0, 1.0].")
        if not (0.0 <= stats.average_validation_score <= 100.0):
            errors.append(f"Statistics Integrity Error: Average validation score '{stats.average_validation_score}' stands outside [0.0, 100.0].")
        if not (0.0 <= stats.average_coverage <= 100.0):
            errors.append(f"Statistics Integrity Error: Average coverage '{stats.average_coverage}' stands outside [0.0, 100.0].")
    ...
    def compile_statistics(self, records: List[ApprovalDecisionRecord]) -> ApprovalStatistics:
        if not records:
            return ApprovalStatistics(
                total_sessions=0, approved_count=0, rejected_count=0,
                changes_requested_count=0, average_confidence=0.0,
                average_validation_score=0.0, average_coverage=0.0
            )

        total = len(records)
        app_cnt = sum(1 for r in records if r.final_state in [ApprovalState.APPROVED, ApprovalState.APPROVED_WITH_CONDITIONS])
...
class LocalApprovalHistoryService(ApprovalHistoryService):
    ...
    def transition_state(self, workspace_id: str, session_id: str, target_state: ApprovalState, actor: str, reason: str) -> ApprovalHistoryEntry:
        ...
        if not self._validator.validate_transition(current_state, target_state):
            raise ValueError(f"Invalid transition state trigger: '{current_state.value}' -> '{target_state.value}'.")
    ...
    def run_history_analysis(self, workspace_id: str) -> ApprovalHistoryReport:
        ...
        trends_md = []
        for t in trends:
            trends_md.append(f"- **{t.metric_name}**: Trend direction evaluates as `{t.direction.upper()}`.")

        patterns_md = []
        for p in patterns:
            patterns_md.append(f"- [{p.pattern_type.upper()}] {p.description} (Occurrences: {p.occurrence_count})")
