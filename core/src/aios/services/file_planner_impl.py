...
class LocalFileImpactAnalyzer(FileImpactAnalyzer):
    ...
    def analyze_impact(
        ...
        if not affected_files and target_files:
            ...
            affected_directories.append(
                AffectedDirectory(
                    dir_path=d, reason="Contains default fallback file.", affected_files_count=1
                )
            )
    ...
    def plan_changes(
        ...
        for af in scope.affected_files:
            ...
            if af.file_path in code_summary.dependency_graph:
                ...
                importers_count = sum(
                    1
                    for target, deps in code_summary.dependency_graph.items()
                    if af.file_path in deps
                )
            ...
            if any(api in af.file_path for api in code_summary.public_apis):
                interfaces.append(
                    f"Public API definitions inside {af.file_path} might break consumers."
                )
        ...
        doc_cp = [
            "Update Knowledge Base reference index.",
            "Update PROJECT_STATUS.md milestone table.",
        ]
...
class LocalFilePlanner(FilePlanner):
    ...
    def generate_planning_result(
        ...
        affected_files, affected_directories = self._analyzer.analyze_impact(
            objective, code_summary
        )
        ...
        direct, indirect, high_risk = self._resolver.resolve_dependencies(
            affected_files, code_summary
        )
        ...
        if self._model:
            try:
                ...
                # Merge LLM feedback
                result.implementation_sequence = data.get(
                    "implementation_sequence", result.implementation_sequence
                )
                result.high_risk_dependencies = data.get(
                    "high_risk_dependencies", result.high_risk_dependencies
                )
                result.circular_dependency_risks = data.get(
                    "circular_dependency_risks", result.circular_dependency_risks
                )
                result.interface_risks = data.get("interface_risks", result.interface_risks)
                result.shared_module_risks = data.get(
                    "shared_module_risks", result.shared_module_risks
                )
                result.configuration_risks = data.get(
                    "configuration_risks", result.configuration_risks
                )
                ...
                result.testing_checkpoints = data.get(
                    "testing_checkpoints", result.testing_checkpoints
                )
                result.documentation_checkpoints = data.get(
                    "documentation_checkpoints", result.documentation_checkpoints
                )
                result.rollback_checkpoints = data.get(
    ...
    def publish_planning_result(self, result: PlanningResult) -> None:
        ...
        for f in result.scope.affected_files:
            files_md.append(
                f"- `{f.file_path}` ({f.modification_type.value.upper()}) [Risk: {f.risk_level}]: {f.reason}"
            )
        ...
        report_md = (
            ...
            + "\n".join(
                [
                    f"{idx}. `{f_path}`"
                    for idx, f_path in enumerate(result.implementation_sequence, 1)
                ]
            )
            + "\n\n"
